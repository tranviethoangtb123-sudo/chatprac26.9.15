    ...
    - 对话：**场景对话库**（`assets/js/data.scenarios.js` + `assets/css/scenarios.css`），
      **两级选择器**：点一下像选国家/日期那样展开全部选项。
      - 界面上只有两级文字：**一级标题是域**（社交人际 / 职场工作 …）**可折叠**，点一下展开收起，
        手风琴式一次只开一个（不然 176 条选项要滑二十多屏）；二级是选项文字 **场景 · 变体**
        （例：`面试 · 顺利达成（对等）`）。同一域里变体名会重复，所以选项必须带场景名才分得清。
        不显示雅思标签、关系/语域/渠道/障碍/结果 元数据、说话人姓名，也没有顶部统计条。
      - **12 个域 / 规划 60 个场景**，当前已收录 22 个场景 × 8 种变体 = **176 段 1926 话轮**。
      ...
      - **朗读**：每句两个按钮 **美 / 英**（en-US / en-GB 两套语音，浏览器语音，离线）。
      - **目标完成框**（选择器下面，选中一段对话后出现）：一个**课题 = 一个场景（8 种变体）**，
        每段后面两个键 —— **掌握**（这段学完了，绿色）/ **练习**（还要再学，黄色），再点一次取消。
        顶部显示「今天掌握 N / 8 · 全部已掌握 M 段」和一条进度条，跨天自动从 0 开始（按日期比较）；
