...
- **两种模式**（侧边栏最下方切换，选择会记住）：
  ...
  - **学习模式**：只有 **单词** 和 **对话** 两个板块。
    - 单词：一套 **6 板块 + 三键循环** 的背词系统（`assets/js/data.vocab.js` 自带 162 词，与查询模式的 4198 词库互相独立）。
      - 六个板块：**收词台**（粘贴导入生词、清空进度）/ **今日新词**（每天上限 50，跨场景域打散混排）/
        **待复习**（当天到期）/ **已掌握**（可「重新学」放回队列）/ **听写轨**（只放音，凭听写拼写）/ **语块库**（词组整句，不进单词表）。
      - 三键循环：**认识**（记忆盒 +1，间隔 1/3/7/14/30 天，满格即已掌握）、**模糊**（盒不动，明天再考）、
        **不认识**（盒归零，10 分钟后重来）。卡片初始只给单词 + 音标 + 词性 + 发音，点「显示释义」后才展开释义、易混提示和固定搭配。
      - **门禁**：待复习没清完时，今日新词会先拦下来（可点「仍要学新词」强行放行）。
      - 发音用浏览器 `speechSynthesis`（en-GB，不联网）；音标和固定搭配从查询模式的数据里借，不重复维护。
      - 学习进度存在 `localStorage` 的 `chatprac-vocab-study`，与查询模式完全独立。
    - 对话：目前留空（内容待定）。
