...
- **两种模式**（侧边栏最下方切换，选择会记住）：
  ...
  - **学习模式**：只有 **单词** 和 **对话** 两个板块。
    - 单词：一套 **列表式背词系统**（`assets/js/data.vocab.js`，162 词，与查询模式的 4198 词库互相独立）。
      ...
      - **学习模式底部没有输入框**（加词/搜索都从学习板块去掉）；底部改成**进度条**，见下面「对话」那节。
      - 进度存在 `localStorage` 的 `chatprac-vocab-study`，与查询模式完全独立。
    - 对话：**场景对话库**（`assets/js/data.scenarios.js` + `assets/css/scenarios.css`），
      **两级选择器**：点一下像选国家/日期那样展开全部选项。
      - 界面上只有两级文字：**一级标题是域**（社交人际 / 职场工作 …）**可折叠**，点一下展开收起，
        手风琴式一次只开一个（不然两百多条选项要滑很久）；二级是选项文字 **场景 · 变体**
        （例：`面试 · 顺利达成（对等）`）。同一域里变体名会重复，所以选项必须带场景名才分得清。
        不显示雅思标签、关系/语域/渠道/障碍/结果 元数据、说话人姓名，也没有顶部统计条。
      - **再点一次已经选中的标题 = 取消选中**：一级标题收起、二级标题恢复原样，下面的对话同时收起；
        选中的一级标题和二级标题行都有高亮样式。
      - **12 个域 / 规划 60 个场景**，当前已收录 28 个场景 × 8 种变体 = **224 段 2437 话轮**。
      - 每个场景固定 **8 种变体**：顺利达成（对等 / 权力不对等）、拒绝或异议→折中、沟通故障→澄清→修复、
        情绪型、渠道变体、高正式语域、低正式语域。第 3~7 段必须带「障碍 → 修复 → 明确结果」，
        只有顺利对话等于只覆盖三成真实互动。
      - 进板块只有「选择场景」；选完立刻呈现整段对话，每个域只有收录了内容才会出现在选项里。
      - **搜索**（底部输入框）命中变体名和对话正文；搜索时自动展开选项并跳到第一段命中。
      - **朗读已去掉**（浏览器自带语音效果太差）；背单词板块点单词发音的功能保留。
      - **掌握 / 练习**就在下拉框**每一行的右边**（不再有单独的目标完成框）：
        一个**课题 = 一个场景（8 种变体）**，**掌握**（学完了，绿）/ **练习**（还要再学，黄），再点一次取消；
        下拉框顶部一行显示「今天掌握 N / 8 · 全部已掌握 M 段」，跨天从 0 开始（按日期比较）；
        进度存在本机 `localStorage` 的 `chatprac-dialogue-goal`，格式 `{ mark: { "s04-01:0": { s, d } } }`。
      - **底部进度条**（学习模式的底部，就是原来输入框的位置）：两条条状进度 ——
        **全场景**（已掌握 / 全部 224 段）和**当前一级标题**（当前域，例如「社交人际 3 / 64」）；
        百分比不到 10% 时保留一位小数；点「掌握」立刻更新。
