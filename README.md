...
tests/sync.js                进度同步测试（导出 → 清空缓存 → 恢复）
```
...
node tests/sync.js        # 进度同步：导出 → 清缓存 → 恢复
...
- 词库按「牛津核心 / 柯林斯星级 / 考纲标签」筛选并按词频排序，**并入了雅思标签词**（词频前 8000 内），
  所以雅思基础词汇基本都在里面。
- **每日任务**：每天发 50 个新词（一天一批，不是一次性）。跨天检查在**每次渲染**和**回到前台**
  （`visibilitychange` / `focus`）都会做一次 —— 装成 App 后是从后台恢复、不会重新加载页面，
  只在加载时查一次的话日期会永远停在昨天。到期复习排在新词前面，**复习不占**今日新词额度；
  统计行显示「今日新词 N/50（还剩 x 个）· 复习到期 · 已学习 · 全部 · 连续 N 天」。
- **进度同步**（侧边栏底部「进度同步」）：进度只存在本机 `localStorage`，清缓存或换手机就没了，
  所以提供三条通道 —— 三条都基于同一个快照（背单词进度 + 对话掌握/练习 + 模式 + 主题）：
  1. **导出到文本 / 从文本恢复**：不依赖网络，复制粘贴即可；
  2. **备份到文件**：手机上存进「文件」App 或网盘；
  3. **GitHub Gist**：粘一个只勾 `gist` 权限的 token（存在本机），进度传到自己的私有 Gist；
     换手机或清缓存后，在同一面板粘同一个 token，点「下载进度」就全部回来（开着自动同步时，
     学一会儿自动上传，启动时自动拉取）。
